/**
 * Weekly Open Mics Digest
 *
 * Business logic for fetching upcoming open mics and building per-user digest emails.
 * Used by the /api/cron/weekly-open-mics endpoint.
 *
 * Phase 1 MVP:
 * - Query/filter: event_type="open_mic", is_published=true, status="active"
...
export interface OpenMicOccurrence {
  event: OpenMicEvent;
  dateKey: string;
...
/**
 * Fetch all open mics that may have occurrences in the date range.
 * Returns raw event data to be expanded into occurrences.
 */
async function fetchOpenMicEvents(
  ...
  if (error) {
    console.error("[WeeklyDigest] Failed to fetch open mics:", error);
    return [];
  }
...
async function fetchCancelledOccurrences(
  ...
  if (error) {
    console.error("[WeeklyDigest] Failed to fetch cancelled occurrences:", error);
    return new Map();
  }
...
/**
 * Get all upcoming open mics for the digest date range.
 * Expands recurring events and filters out cancelled occurrences.
 */
export async function getUpcomingOpenMics(
  supabase: SupabaseClient<Database>,
  options?: { todayKey?: string }
): Promise<DigestData> {
  const { start, end } = getDigestDateRange(options?.todayKey);

  // Fetch all open mic events
  const events = await fetchOpenMicEvents(supabase);
  ...
  const byDate = new Map<string, OpenMicOccurrence[]>();
  ...
  for (const event of events) {
    ...
    for (const occurrence of occurrences) {
      ...
      const openMicOccurrence: OpenMicOccurrence = {
        event,
      ...
      byDate.get(occurrence.dateKey)!.push(openMicOccurrence);
...
export async function getDigestRecipients(
  ...
  if (profilesError) {
    console.error("[WeeklyDigest] Failed to fetch profiles:", profilesError);
    return [];
  }
