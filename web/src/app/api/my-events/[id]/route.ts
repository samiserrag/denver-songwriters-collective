...
export async function PATCH(
  ...
  const { data: currentEvent } = await supabase
    .from("events")
    .select("has_timeslots, total_slots")
    .eq("id", eventId)
    .single();
  ...
  // Safe timeslot regeneration: only regenerate if timeslots enabled and no existing claims
  ...
  if (timeslotsNowEnabled && (timeslotsWereDisabled || totalSlotsChanged)) {
    // Check for existing claimed timeslots before regenerating
    ...
    if (existingSlots && existingSlots.length > 0) {
      ...
      if (claimCount && claimCount > 0) {
        // Has existing claims - do NOT regenerate, just update the event fields
        console.log(`[PATCH /api/my-events/${eventId}] Skipping timeslot regeneration: ${claimCount} existing claims`);
      } else {
        // No claims - safe to regenerate
