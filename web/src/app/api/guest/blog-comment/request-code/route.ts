...
/**
 * POST /api/guest/blog-comment/request-code
 *
 * Request a verification code for guest comment on a blog post.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { post_id, guest_name, guest_email, content, parent_id } = body;

    if (!post_id || !guest_name || !guest_email || !content) {
    ...
    const emailContent = getVerificationCodeEmail({
      ...
      eventTitle: post.title || "Blog Post",
      code,
      expiresInMinutes: CODE_EXPIRES_MINUTES,
      purpose: "blog_comment",
    });
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.log(`[DEV] Blog comment verification code for @${domain}: ${code}`);
    }
    ...
    console.error("Request blog comment code error:", error);
    return NextResponse.json(
