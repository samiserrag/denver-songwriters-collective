...
export default function MyEventsFilteredList({ events: initialEvents, isApprovedHost }: Props) {
  ...
  // Phase 5.14b: Filter events based on active tab (3 tabs now)
          ...
          {filteredEvents.map((event) => {
            ...
            return (
              <Link
                ...
                className={`block p-6 border rounded-lg transition-colors ${
                  isCancelled
                    ? "bg-[var(--color-bg-secondary)]/50 hover:bg-[var(--color-bg-secondary)] border-[var(--color-border-default)] opacity-70 hover:opacity-90"
                    : "bg-[var(--color-bg-secondary)] hover:bg-[var(--color-bg-tertiary)] border-[var(--color-border-default)] hover:border-[var(--color-border-accent)]"
                }`}
              >
                <div className="flex items-start gap-4">
                  {/* Date box - muted for cancelled */}
                  {event.event_date && (
                  ...
                  <div className="flex-1">
                    ...
                    <h2 className={`text-lg text-[var(--color-text-primary)] font-medium ${
