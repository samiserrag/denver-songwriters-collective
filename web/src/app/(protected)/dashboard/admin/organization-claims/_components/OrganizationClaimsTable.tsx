...
export function OrganizationClaimsTable({ claims, showActions = true }: Props) {
  ...
  const handleApprove = async (claim: OrganizationClaim) => {
    setLoadingId(claim.id);
    setError(null);

    try {
      const response = await fetch(`/api/admin/organization-claims/${claim.id}/approve`, {
        method: "POST",
      });
      ...
      console.error("Approve organization claim error:", err);
  ...
  const handleReject = async (claimId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/organization-claims/${claimId}/reject`, {
        method: "POST",
      ...
      console.error("Reject organization claim error:", err);
      setError("An unexpected error occurred");
    } finally {
      setLoadingId(null);
      setRejectingId(null);
              ...
              >
                  ...
                  {claim.organization ? (
                    <div>
                      ...
                      <p className="text-xs text-[var(--color-text-tertiary)]">
                        {(claim.organization.organization_type || "Organization") +
                          (claim.organization.city ? ` • ${claim.organization.city}` : "")}
                      </p>
                    </div>
                  ) : (
                ...
                <td className="py-3 px-4">
                  {claim.requester ? (
                    ...
                    <span className="text-[var(--color-text-tertiary)] italic">User deleted</span>
                  )}
                </td>
                <td className="py-3 px-4 max-w-xs">
                  {claim.message ? (
                    <p className="text-[var(--color-text-secondary)] truncate">{claim.message}</p>
                  ) : (
                    <span className="text-[var(--color-text-tertiary)] italic">No message</span>
                  )}
                </td>
