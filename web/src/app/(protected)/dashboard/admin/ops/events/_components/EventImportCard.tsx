...
export default function EventImportCard() {
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await fetch("/api/admin/ops/events/preview", {
        method: "POST",
  ...
  const handleApply = async () => {
    ...
    try {
      const response = await fetch("/api/admin/ops/events/apply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ csv: csvContent }),
      });
  ...
  return (
    <div className="p-6 bg-[var(--color-bg-secondary)] border border-[var(--color-border-default)] rounded-lg">
      ...
      <p className="text-[var(--color-text-secondary)] text-sm mb-4">
        Upload an edited CSV to preview and apply changes. Only existing events
        will be updated (no new events created). Verification timestamps are
        managed via bulk actions below.
      </p>
      ...
      {/* Diff Preview */}
      {previewResult?.valid && previewResult.updates && (
        <div className="mt-6">
          ...
          <EventDiffTable
            ...
            notFound={previewResult.notFound || []}
          ...
          {canApply && !showConfirm && (
            <div className="mt-4">
              ...
              >
                Apply {previewResult.updates.length} Updates
              </button>
            </div>
          )}
      ...
      {applyResult && (
        <div className="mt-4 p-4 bg-green-500/10 border border-green-500/30 rounded">
          <p className="text-green-400 font-medium">
            {applyResult.success ? "Changes applied successfully!" : "Completed with errors"}
          </p>
          <p className="text-green-400 text-sm mt-2">
            Updated: {applyResult.summary.updated} • Unchanged:{" "}
            {applyResult.summary.unchanged}
            {applyResult.summary.notFound > 0 &&
              ` • Not found: ${applyResult.summary.notFound}`}
          </p>
