...
| INTERPRETER-13 | Complete Phase 9 monitoring checkpoints and rollout decision memo | P0 | OPEN | **Acceptance criteria:** (1) Execute T+24h and T+7d Axiom queries from `docs/investigation/interpreter-phase9-ux-optimization-stopgate.md`; (2) record completion rate, median/p90 turns, fallback click rate, and 5xx count; (3) fill Day-7 GO/HOLD/ROLLBACK template with concrete numbers and decision rationale; (4) update INTERPRETER-10 status accordingly. |
| INTERPRETER-14 | Host-facing conversational UX naming + reply flow polish | P1 | OPEN | **Acceptance criteria:** (1) Replace confusing host-facing naming so users do not see "Interpreter Lab" language on host entry surfaces; (2) clarify turn flow so assistant replies/questions are visually coupled to the input area (no ambiguous "response below" behavior); (3) preserve existing 8A–9D safeguards and create/apply functionality; (4) add regression tests and production smoke evidence for desktop + mobile. |
