...
| INTERPRETER-10 | Interpreter UX optimization + trust metrics (Phase 9A->9E) | P1 | IN MONITORING (Day 4/7) | Implemented 2026-03-01. 9A-9E deployed + production smoke passed. T+72h checkpoint captured on 2026-03-04 (actual catch-up for missed T+24/T+48 targets). Monitoring window active through 2026-03-08. Stop-gate: `docs/investigation/interpreter-phase9-ux-optimization-stopgate.md`. |
...
| INTERPRETER-12 | Non-owner 403 production smoke for occurrence apply | P1 | DONE — production 403 verified | Both `/api/events/interpret` and `/api/my-events/:id/overrides` return HTTP 403 `{ "error": "Forbidden" }` for non-owner member account. Lab debug panel shows 403; no user-facing toast (acceptable for lab surface). Evidence: `docs/investigation/interpreter-phase9-ux-optimization-stopgate.md` § INTERPRETER-12. |
| INTERPRETER-13 | Complete Phase 9 monitoring checkpoints and rollout decision memo | P0 | IN PROGRESS — monitoring-only | **Progress:** T+72h checkpoint recorded on 2026-03-04 with sample gate met (`23` host impressions, `11` host create sessions), turns/fallback metrics logged, and zero interpret error logs observed. **Remaining:** Day-7 checkpoint and GO/HOLD/ROLLBACK decision memo on 2026-03-08. All implementation work in this tract is complete; this item is decision monitoring only. |
| INTERPRETER-14 | Host-facing conversational UX naming + reply flow polish | P1 | DONE — production smoke passed | Title → "Create Happening with AI", 3-step subtitle cue, two-state button (Generate Draft / Send Answer), conversation history de-emphasized for host. 16 source assertions. Desktop + 375px + 360px smoke passed. No regressions (4958 tests green). |
...
| UX-14 | Admin user toggles lack visible success/error feedback and host-sync consistency | P1 | DONE — commits `86863282`, `bf4f132f` | Added visible success/error notice for host/artist/host/studio/admin toggles in admin user management UI. `toggleHostStatus` now syncs `approved_hosts` (`active`/`revoked`) + `profiles.is_host` together. Follow-up hardening widened Host-column toggle coverage to host-only users, added server-side spotlight eligibility validation in `updateSpotlightType`, and aligned Artist Spotlight eligibility to artist identities (`is_songwriter OR is_host`) rather than songwriter-only. Regression assertions: `web/src/__tests__/admin-user-toggle-feedback-and-sync.test.ts`, `web/src/__tests__/admin-users-spotlight-eligibility.test.ts`. |
| HOST-ROLE-01 | Host role invariants: orphaned cohosts, race conditions, capability sync | P1 | DONE — commit `c8a8cd67` | Token-invite auto-role, atomic CAS on accept, approved_hosts sync, 4 orphaned events repaired, 16 new tests. Evidence: `docs/investigation/host-role-01-invariants-stopgate.md`. |
| HOST-ROLE-02 | Claims-only host ownership path (block self-invite host assignment bypasses) | P0 | DONE — commit `86863282` | Unclaimed events can no longer assign host ownership via invite/cohost flows. Direct host invites + host invite acceptance now return 409 and require admin-approved claim workflow. Cohost invites on orphaned events are blocked until primary host is set via claim approval. Regression assertions: `web/src/__tests__/host-role-01-invariants.test.ts`, `web/src/__tests__/host-invite-auto-role.test.ts`. |
| HOST-ROLE-03 | Admin email for all host claims (event claims + host requests) | P1 | DONE — commits `eb45ba9f`, `86863282` | Event claim submissions fan out admin emails to all admin recipients (dedup + fallback). Platform host request submissions now also fan out admin notification emails to all admins. Tests: `web/src/__tests__/event-claim-admin-email-fanout.test.ts`, `web/src/__tests__/host-requests-admin-email-fanout.test.ts`. |
