...
| UX-14 | Admin user toggles lack visible success/error feedback and host-sync consistency | P1 | DONE — commit `86863282` | Added visible success/error notice for host/artist/host/studio/admin toggles in admin user management UI. `toggleHostStatus` now syncs `approved_hosts` (`active`/`revoked`) + `profiles.is_host` together. Regression assertions: `web/src/__tests__/admin-user-toggle-feedback-and-sync.test.ts`. |
